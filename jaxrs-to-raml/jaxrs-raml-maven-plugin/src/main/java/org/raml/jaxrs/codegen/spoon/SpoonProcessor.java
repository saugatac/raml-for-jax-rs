...
public class SpoonProcessor {
	...
	/**
	 * <p>
	 * Constructor for SpoonProcessor.
	 * </p>
	 *
	 * @param factory
	 *            a {@link spoon.reflect.factory.Factory} object.
	 */
	...
	private void adjustReturnedAndBodyType(IMethodModel method_, boolean hasGlobalConsumes) {
		...
		for (IParameterModel param_ : parameters) {
			...
			// type.getAnnotation(JAVAX_XML_TYPE);
	...
	private static HashSet<String> primitives = new HashSet<String>(Arrays.asList("byte", "java.lang.Byte", "short",
			"java.lang.Short", "int", "java.lang.Integer", "long", "java.lang.Long", "float", "java.lang.Float",
			"double", "java.lang.Double", "character", "java.lang.Character", "boolean", "java.lang.Boolean"));
	 ...
	 *
	 * @param classElement
	 *            a {@link spoon.reflect.declaration.CtType} object.
	 */
	...
	private ITypeModel processType(CtType<?> classElement) {
		...
		if (classElement instanceof CtType) {
			...
			Set<CtMethod<?>> methods = ((CtType<?>) classElement).getAllMethods();
	...
	private void fillTypeParameters(GenericElementModel model, CtGenericElement element) {
		...
		if (ftp == null || ftp.isEmpty()) {
			if (element instanceof CtType && ((CtType) element).getSuperclass() != null)
				ftp = ((CtType) element).getSuperclass().getActualTypeArguments();
			if (ftp == null || ftp.isEmpty())
				return;
		}
		for (CtTypeReference<?> param : ftp) {
			...
			paramModel.setFullname(param.getQualifiedName());
		...
		for (Map.Entry<String, Object> entry : elementValues.entrySet()) {
			...
			if (annotationList != null) {
				...
				if (value instanceof CtLiteral<?>) {
					...
					// if field references a static final String, use string's
					// value
					if (member instanceof Field) {
						...
						if (Modifier.isStatic(mod) && Modifier.isFinal(mod)) {
							...
							try {
								...
								// NOOP tolerate any exception/error, and fall
								// back to using name of field below
							}
						}
	...
	private IMethodModel processMethod(CtMethod<?> m, TypeModel ownerType) {
		...
		if (returnedType.getQualifiedName().equalsIgnoreCase("T")) {
			if (ownerType.getTypeParameters() != null && ownerType.getTypeParameters().size() == 1) {
				returnedType = new CtTypeReferenceImpl();
				returnedType.setSimpleName(ownerType.getTypeParameters().get(0).getFullname());
			}
		}
	...
	private ITypeModel processTypeReference(CtTypeReference<?> typeReference) {
		...
		try {
			Collection<CtExecutableReference<?>> methods = typeReference.getAllExecutables();
			for (CtExecutableReference<?> m : methods) {
				IMethodModel methodModel = processMethodReference(m);
				type.addMethod(methodModel);
			}
		} catch (Exception e) {
//			System.out.println("ERR:getAllExecutables:" + typeReference.getQualifiedName());
			// e.printStackTrace();
		}
		try {
...
//			System.out.println("ERR:getDeclaredFields:" + typeReference.getQualifiedName());
			// e.printStackTrace();
		}
		return new ProxyType(registry, qualifiedName);
	}
...
//			System.out.println("ERR:getActualClass+" + type.getQualifiedName());
		}
	}
	...
	private IParameterModel processParameterReference(CtTypeReference<?> paramTypeReference) {
		...
		List<CtAnnotation<? extends Annotation>> annotations = paramTypeReference.getTypeAnnotations();
		for (CtAnnotation a : annotations) {
			IAnnotationModel annotationModel = processJavaLangAnnotation(a.getActualAnnotation());
	...
	private void fillReference(BasicModel model, CtReference ref) {
		...
		try {
			List<CtAnnotation<? extends Annotation>> annotations2 = ref.getDeclaration().getAnnotations();
			annotations = new LinkedList<Annotation>();
			for (CtAnnotation ca : annotations2) {
				annotations.add(ca.getActualAnnotation());
			}
		} catch (Exception e) {
		}
